...
psr.add_argument('-z', dest='zRange', nargs='+', help="zrange")
...
args = psr.parse_args()
order=args.ipt
zRange = [np.double(i) for i in args.zRange]
print(zRange)
peFiles = args.pe
timeFiles = args.time
...
for zr in zRange:
    ...
    for i in np.arange(np.size(coeff_pe[:,0])):
        ...
        plt.title('pe:{}-th Legendre coeff;z<{}'.format(i, zr))
    ...
    for i in np.arange(np.size(coeff_time[:,0])):
        ...
        plt.ylabel('Coefficients')
        plt.title('time:{}-th Legendre coeff;z<{}'.format(i, zr))
        plt.legend(['raw','fit_inner', 'fit_outer'])
        pdf.savefig()
        #plt.show()
        plt.close()
